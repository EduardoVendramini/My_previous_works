...
############### DADOS ################
data_sem_correcao = pd.read_excel ('C:/Users/eduardo/Documents/GitHub/Rotas_Via_Lacteos/Eduardo/Comparacoes/Rotas_1/Automatico_Sem_Correcao.xlsx')
# Exclui linhas média e info do final
data_sem_correcao.drop(data_sem_correcao[(data_sem_correcao['id_rota'])=='Média'].index, inplace=True)
data_sem_correcao.drop(data_sem_correcao[(data_sem_correcao['id_rota'])=='Info'].index, inplace=True)
# Estatística dos dados
describe= data_sem_correcao.describe()


# Se não tiver medição de tempo real exclui linha
#data_sem_correcao.drop(data_sem_correcao[(data_sem_correcao['Tempo real']=='?')].index, inplace=True)
# Tempo vira float
# data_sem_correcao = data_sem_correcao.astype({'Tempo real':'float'})
